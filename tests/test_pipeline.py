...
from regulatory_tools.traceability.generator import apply_test_markers, build_trace_matrix
...
def test_pipeline_execution(tmp_path, evidence_output_dir):
    report = EvidenceReport(
        subject="generate_traceability_matrix produces traceability_matrix.md from requirements.yaml"
    )
...
"""
    ...
    report.info("documentation artifact produced from pipeline", "DOC-003")
    report.info("pipeline orchestration completed", "SYS-002")
...
def test_traceability_cli_execution(tmp_path, evidence_output_dir):
    report = EvidenceReport(
        subject="generate_traceability_matrix executes deterministically and produces matrix output"
    )
...
"""
    ...
    report.info("deterministic pipeline run completed", "SYS-002")
...
def test_scan_tests_detects_tests(tmp_path, evidence_output_dir):
    report = EvidenceReport(
        subject="collect_requirement_markers detects @pytest.mark.requirement annotations in test files"
    )
...
"""
    ...
    report.info(
        f"VER-001 detected={('VER-001' in tests)}, test_count={len(tests.get('VER-001', []))}",
        "INF-004",
    )
...
def test_run_tests_and_trace_smoke(tmp_path, evidence_output_dir):
    ...
    report = EvidenceReport(
        subject="run_tests_and_trace executes end-to-end in a CI-compatible environment"
    )
...
def test_requirement_markers_count_as_linked_coverage(tmp_path, evidence_output_dir):
    report = EvidenceReport(
        subject="@pytest.mark.requirement markers produce LINKED status; untouched requirements stay UNTESTED"
    )
...
"""
    ...
    report.auto_save(
        "ver002_ver005_requirement_markers_count_as_linked_coverage", evidence_output_dir
    )
    assert not report.has_errors, report.summary()
...
def test_run_pytest_with_coverage_uses_active_python(tmp_path, monkeypatch, evidence_output_dir):
    report = EvidenceReport(
        subject="run_pytest_with_coverage invokes pytest via the active Python interpreter"
    )
