...
def test_full_traceability_workflow(tmp_path: Path, evidence_output_dir):
    ...
    report = EvidenceReport(
        subject="Full traceability workflow: requirements → evidence → matrix → markdown"
    )
    ...
    missing, untracked = validate_traceability(
        requirements_yaml=requirements_yaml, test_dir=tests_dir
    )
    ...
    report.info(
        f"matrix_rows={len(matrix)}, VER-001_PASS={matrix_by_id['VER-001']['status'] == 'PASS'}",
        "VER-005",
    )
    report.info(f"markdown_output_exists={output_md.exists()}", "DOC-003")
    report.info(f"artifact_persisted={output_md.exists()}", "INF-003")
    report.auto_save(
        "ver002_ver003_ver005_doc003_inf003_full_traceability_workflow", evidence_output_dir
    )
...
def test_traceability_matrix_determinism(tmp_path: Path, evidence_output_dir):
    ...
    report = EvidenceReport(
        subject="build_trace_matrix produces identical output for identical inputs"
    )
    ...
    report.info("deterministic_output_confirmed", "SYS-002")
...
def test_invalid_yaml_rejected(tmp_path: Path, evidence_output_dir):
    ...
    report = EvidenceReport(
        subject="validate_traceability raises a typed exception on malformed YAML"
    )
...
def test_unknown_requirement_in_evidence(tmp_path: Path, evidence_output_dir):
    report = EvidenceReport(
        subject="build_trace_matrix completes without crashing when evidence references unknown requirement IDs"
    )
...
@pytest.mark.requirement("VER-002")
def test_evidence_issue_requirement_ids_link_matrix_when_top_level_missing(
    tmp_path: Path, evidence_output_dir
):
    report = EvidenceReport(
        subject="build_trace_matrix uses issue-level requirement IDs when top-level requirements list is empty"
    )
...
def test_empty_evidence_directory(tmp_path: Path, evidence_output_dir):
    report = EvidenceReport(
        subject="build_trace_matrix marks all requirements UNTESTED when evidence directory is empty"
    )
    ...
    report.info(
        f"all_untested={all(s == 'UNTESTED' for s in statuses)}, statuses={statuses}", "VER-003"
    )
...
def test_invalid_requirement_id_format(tmp_path: Path, evidence_output_dir):
    report = EvidenceReport(
        subject="validate_traceability raises on requirement IDs that do not match the DOMAIN-NNN format"
    )
...
def test_requirement_validation(tmp_path, evidence_output_dir):
    report = EvidenceReport(
        subject="validate_traceability detects orphaned requirements and verifies requirement ID format"
    )
...
def test_evidence_summary_generation(tmp_path, evidence_output_dir):
    report = EvidenceReport(
        subject="generate_evidence_summary aggregates evidence artifacts including risk classification metadata"
    )
    ...
    report.info("risk_metadata_preserved_in_summary", "RSK-002")
...
def test_code_coverage_computation(tmp_path, evidence_output_dir):
    report = EvidenceReport(
        subject="compute_code_coverage parses coverage.xml and returns accurate line coverage percentage"
    )
    ...
    report.info(
        f"coverage_pct={coverage_pct}, matches_0.85={abs(coverage_pct - 85.0) < 0.1}", "INF-003"
    )
    report.info("deterministic_coverage_result", "SYS-002")
...
def test_traceability_cli_execution(tmp_path, evidence_output_dir):
    report = EvidenceReport(
        subject="regulatory_tools.traceability CLI executes with exit code 0 in CI environment"
    )
...
def test_build_trace_matrix_row_has_source_file_field(tmp_path, evidence_output_dir):
    ...
    report = EvidenceReport(
        subject="DHF-012: each matrix row has source_file key set to filename stem"
    )
    ...
    report.info(
        f"source_file_key_present={'source_file' in matrix[0]}, value={matrix[0].get('source_file')}",
        "DHF-012",
    )
...
def test_load_requirements_stores_extended_fields(tmp_path, evidence_output_dir):
    ...
    report = EvidenceReport(
        subject="DHF-012: load_requirements stores extended fields with correct defaults"
    )
...
def test_write_markdown_includes_extended_column_headers(tmp_path, evidence_output_dir):
    ...
    report = EvidenceReport(
        subject="DHF-012: write_markdown output includes all extended column headers"
    )
    ...
    report.info(
        f"all_extended_headers_present={all(h in text for h in ['Derived From', 'Design Spec', 'Risk Ref', 'Verification Method', 'Safety Relevant'])}",
        "DHF-012",
    )
...
def test_covered_status_single_level(tmp_path: Path, evidence_output_dir):
    ...
    report = EvidenceReport(
        subject="DHF-012: parent requirement becomes COVERED when single child is PASS"
    )
    ...
    report.info(
        f"SYS-001_PASS={by_id['SYS-001']['status'] == 'PASS'}, UN-001_COVERED={by_id['UN-001']['status'] == 'COVERED'}",
        "DHF-012",
    )
...
def test_covered_status_mixed_children_stays_untested(tmp_path: Path, evidence_output_dir):
    ...
    report = EvidenceReport(
        subject="DHF-012: parent stays UNTESTED when not all children are PASS/COVERED"
    )
    ...
    report.info(
        f"SYS-002_UNTESTED={by_id['SYS-002']['status'] == 'UNTESTED'}, UN-001_UNTESTED={by_id['UN-001']['status'] == 'UNTESTED'}",
        "DHF-012",
    )
...
def test_covered_status_no_children_stays_untested(tmp_path: Path, evidence_output_dir):
    ...
    report = EvidenceReport(
        subject="DHF-012: isolated requirement with no children and no evidence stays UNTESTED"
    )
...
def test_covered_status_two_level_transitive(tmp_path: Path, evidence_output_dir):
    ...
    report = EvidenceReport(
        subject="DHF-012: COVERED propagates transitively through multi-level requirement hierarchy"
    )
    ...
    report.info(
        f"SYS-001_COVERED={by_id['SYS-001']['status'] == 'COVERED'}, UN-001_COVERED={by_id['UN-001']['status'] == 'COVERED'}",
        "DHF-012",
    )
...
def test_covered_counts_toward_requirement_coverage(tmp_path: Path, evidence_output_dir):
    ...
    report = EvidenceReport(
        subject="DHF-012: COVERED requirements count toward coverage percentage"
    )
