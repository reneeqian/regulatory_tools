...
def test_known_placeholder_is_replaced(evidence_output_dir):
    report = EvidenceReport(
        subject="DHF-002: PlaceholderFiller replaces known {{VAR}} with the context value"
    )
    ...
    report.info(
        f"{{{{PROJECT_NAME}}}} → content contains 'COCA-prj'; filled={result.filled}", "DHF-002"
    )
...
def test_unknown_placeholder_is_left_unchanged(evidence_output_dir):
    report = EvidenceReport(
        subject="DHF-002: PlaceholderFiller leaves unknown {{VAR}} unchanged and reports it"
    )
...
def test_filled_value_is_wrapped_in_sentinel_comments(evidence_output_dir):
    report = EvidenceReport(
        subject="DHF-010: Filled value is wrapped in <!-- DHF_VAR:name --> sentinels"
    )
...
def test_stale_sentinel_value_updated_on_reraise(evidence_output_dir):
    report = EvidenceReport(
        subject="DHF-010: Re-fill updates sentinel-wrapped value when context changes"
    )
    ...
    report.info("After context change: content updated to 'new-name'", "DHF-010")
...
def test_manual_fill_without_sentinel_is_not_overwritten(evidence_output_dir):
    report = EvidenceReport(
        subject="DHF-010: Manually filled value without sentinel is never touched by the filler"
    )
...
def test_fill_is_idempotent_when_context_unchanged(evidence_output_dir):
    report = EvidenceReport(
        subject="DHF-006: Filling the same content twice with same context produces identical output"
    )
...
def test_fill_file_writes_in_place(tmp_path, evidence_output_dir):
    ...
    report.info("fill_file wrote sentinel-wrapped value to disk", "DHF-002")
...
def test_fill_file_does_not_write_when_content_unchanged(tmp_path, evidence_output_dir):
    report = EvidenceReport(
        subject="DHF-006: fill_file() skips write when content would not change (avoids spurious diffs)"
    )
    ...
    assert doc.stat().st_mtime == original_mtime, (
        "File was written even though content did not change"
    )
